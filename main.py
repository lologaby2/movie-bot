import os
import telebot
from movie_pipeline import process_tiktok_video
from video_generator import create_final_video
...
def start_message(message):
    bot.send_message(message.chat.id, "👋 مرحبًا بك! أرسل رابط فيديو تيك توك لبدء المعالجة.")

@bot.message_handler(func=lambda msg: msg.text and 'tiktok.com' in msg.text)
def handle_tiktok_link(message):
    link = message.text
    bot.send_message(message.chat.id, "📥 جاري تحميل الفيديو وتحليله...")
    try:
        result = process_tiktok_video(link, message.chat.id)
...
def handle_audio(message):
    ...
    try:
        with open(f"waiting_audio/{user_id}.txt") as f:
            video_id = f.read().strip()
    except FileNotFoundError:
        return bot.send_message(user_id, "❗ لم يتم العثور على سياق سابق. أرسل رابط تيك توك أولاً.")
